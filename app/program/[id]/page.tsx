...
import { ShieldCheck, Heart, MapPin, ArrowRight, Loader2, Copy, CheckCircle2 } from 'lucide-react';
        ...
        <div className="grid lg:grid-cols-3 gap-10 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-[2rem] p-6 md:p-8 shadow-xl border border-slate-200/60">
              <span className="inline-block px-3 py-1 bg-teal-50 text-teal-700 font-bold text-[10px] rounded-lg uppercase tracking-wider mb-4 border border-teal-100">
                {program.category}
              </span>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 mb-4 tracking-tight">{program.title}</h1>
              <div className="text-sm md:text-base text-slate-600 space-y-4 whitespace-pre-line leading-relaxed">{program.description}</div>
            </div>
          </div>

          <div className="lg:col-span-1">
            <form onSubmit={handleDonateSubmit} className="bg-white rounded-[2rem] p-6 shadow-2xl border border-slate-200/60 space-y-5">
              <div>
                <h3 className="font-black text-slate-900 text-base">Donasi Transfer Manual</h3>
                <p className="text-xs text-slate-400 mt-0.5">100% donasi masuk tanpa potongan biaya admin pihak ketiga.</p>
              </div>

              <div className="bg-slate-50 rounded-2xl p-4 border text-xs font-semibold">
                <div className="flex justify-between items-center mb-2">
                  <div>
                    <span className="text-slate-400 block mb-0.5 uppercase tracking-wider">Terkumpul</span>
                    <span className="text-lg font-black text-teal-600">Rp {(program.terkumpul || 0).toLocaleString('id-ID')}</span>
                  </div>
                  <span className="font-bold text-slate-500">Target Rp {program.target.toLocaleString('id-ID')}</span>
                </div>
              ...
              {/* JIKA TARGET TERPENUHI, SEMBUNYIKAN FORM DAN TAMPILKAN PESAN INI */}
              {isTargetReached ? (
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 text-center space-y-3">
                  <div className="w-12 h-12 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-2">
                    <CheckCircle2 className="w-6 h-6" />
                  </div>
                  <h4 className="font-black text-emerald-800 text-base">Alhamdulillah, Target Terpenuhi!</h4>
                  <p className="text-xs font-medium text-emerald-700/80 leading-relaxed">
                    Penggalangan dana untuk program ini telah ditutup karena target telah tercapai atau program telah selesai. Terima kasih atas kedermawanan Anda.
                  </p>
                ...
                /* JIKA TARGET BELUM PENUH, TAMPILKAN FORM SEPERTI BIASA */
                <>
                  <div className="space-y-2 border-t border-slate-100 pt-4">
                    <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">1. Masukkan Nominal Donasi</span>
                    ...
                    <div className="relative mt-2">
                      ...
                      {/* INPUT TEXT DENGAN AUTO FORMAT TITIK */}
                      <input 
                        type="text" 
                        placeholder="Minimal 10.000" 
                        value={customAmount} 
                        onChange={handleCustomInput} 
                        className="w-full bg-slate-50 border rounded-xl py-2.5 pl-9 pr-4 text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500" 
                      />
                    </div>
                  </div>

                  <div className="space-y-2 pt-2">
                    <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">2. Identitas Donatur</span>
                    <input type="text" placeholder="Nama Lengkap" disabled={isAnon} value={isAnon ? 'Hamba Allah' : donorName} onChange={(e) => setDonorName(e.target.value)} className="w-full bg-slate-50 border rounded-xl py-2.5 px-3 text-xs font-medium focus:outline-none disabled:opacity-60" />
                    <input type="tel" required placeholder="Nomor WhatsApp (Wajib)" value={donorPhone} onChange={(e) => setDonorPhone(e.target.value)} className="w-full bg-slate-50 border rounded-xl py-2.5 px-3 text-xs font-medium focus:outline-none" />
                    <label className="flex items-center gap-2 cursor-pointer pt-0.5">
                      <input type="checkbox" checked={isAnon} onChange={(e) => setIsAnon(e.target.checked)} className="rounded text-teal-600 w-3.5 h-3.5" />
                      <span className="text-[11px] font-semibold text-slate-500">Sembunyikan nama (Hamba Allah)</span>
                    </label>
                  </div>

                  <div className="space-y-2 pt-2 pb-2">
                    <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">3. Transfer ke Rekening Yayasan</span>
                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-3">
                      {bankAccounts.map((b) => (
                        <div key={b.bank} className="flex justify-between items-center">
                          <div>
                            <span className="font-black text-slate-800 text-sm block leading-none">{b.bank}</span>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button type="button" onClick={() => copyToClipboard(b.acc, b.bank)} className="p-1.5 bg-white border border-slate-200 rounded-lg text-slate-400 hover:text-teal-600 transition-colors">
                  ...
                  <div className="pt-2">
                    <button type="submit" disabled={submitLoading || finalAmount < 10000} className="w-full py-4 bg-[#25D366] hover:bg-[#1EBE5D] disabled:bg-slate-200 disabled:text-slate-400 text-white font-black rounded-xl shadow-lg transition-all text-xs md:text-sm flex items-center justify-center gap-2">
